...
-- Trash Panda Loops - Full Database Reset and Configuration
-- ============================================
-- Run this ENTIRE file in Supabase SQL Editor
-- This will create everything needed for the game to work
-- ============================================
...
-- ============================================
-- STEP 6: CREATE USER_ACHIEVEMENTS TABLE
-- ============================================
...
-- ============================================
-- STEP 7: CREATE USER_PROGRESS TABLE
-- ============================================
...
-- ============================================
-- STEP 8: CREATE GAME_SESSIONS TABLE
-- ============================================
...
-- ============================================
-- STEP 9: CREATE LEADERBOARD_ENTRIES TABLE
-- ============================================
...
CREATE INDEX idx_game_sessions_level_id ON public.game_sessions(level_id);
CREATE INDEX idx_user_progress_user_id ON public.user_progress(user_id);
CREATE INDEX idx_user_progress_level_id ON public.user_progress(level_id);
CREATE INDEX idx_user_achievements_user_id ON public.user_achievements(user_id);
CREATE INDEX idx_leaderboard_entries_level_score ON public.leaderboard_entries(level_id, score DESC);

-- ============================================
-- STEP 11: DISABLE RLS (For Testing)
-- ============================================
...
-- ============================================
-- STEP 13: INSERT 17 ACHIEVEMENTS
-- ============================================
...
-- ============================================
-- STEP 14: CREATE AUTO-PROFILE FUNCTION
-- ============================================
...
-- ============================================
-- STEP 17: UNLOCK LEVEL 1 FOR ALL EXISTING USERS
-- ============================================
...
-- ============================================
-- STEP 19: VERIFICATION AND SUMMARY
-- ============================================
...
BEGIN
  ...
  RAISE NOTICE '═══════════════════════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE 'Tables Created: % of 7', v_tables;
  RAISE NOTICE 'Levels Inserted: % (expected 10)', v_levels;
  RAISE NOTICE 'Achievements Inserted: % (expected 17)', v_achievements;
  RAISE NOTICE 'Users in System: %', v_users;
  RAISE NOTICE 'Profiles Created: %', v_profiles;
  RAISE NOTICE 'Auto-Profile Trigger: %', CASE WHEN v_trigger_exists THEN 'ENABLED ✅' ELSE 'MISSING ❌' END;
  ...
  RAISE NOTICE '═══════════════════════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE '1. Refresh your browser (http://localhost:5173/)';
  RAISE NOTICE '2. Click "Sign In / Play"';
  RAISE NOTICE '3. Log in with your existing account';
  RAISE NOTICE '4. Should work now! ✅';
  RAISE NOTICE '';
  RAISE NOTICE 'If still issues:';
  RAISE NOTICE '- Check browser console (F12) for errors';
  RAISE NOTICE '- Verify your user ID exists in profiles table';
